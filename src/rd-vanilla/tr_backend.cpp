...
void RB_RenderDrawSurfList( drawSurf_t *drawSurfs, int numDrawSurfs ) {
	...
	for (i = 0, drawSurf = drawSurfs ; i < numDrawSurfs ; i++, drawSurf++)
	{
		...
		if ( g_bRenderGlowingObjects && !shader->hasGlow && r_DynamicGlow->integer != 3 )
		{
			shader = oldShader;
...
void RE_StretchRaw (int x, int y, int w, int h, int cols, int rows, const byte *data, int client, qboolean dirty)
{
	...
	if ( cols != tr.scratchImage[client]->width || rows != tr.scratchImage[client]->height ) {
		...
		glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, glConfig.clampToEdgeAvailable ? GL_CLAMP_TO_EDGE : GL_CLAMP );
		glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, glConfig.clampToEdgeAvailable ? GL_CLAMP_TO_EDGE : GL_CLAMP );
	} else {
		if (dirty) {
...
const void	*RB_DrawSurfs( const void *data ) {
	...
	*/
	...
	if ( !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && g_bDynamicGlowSupported && r_DynamicGlow->integer )
	{
		...
		glCopyTexSubImage2D( GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 0, 0, glConfig.vidWidth, glConfig.vidHeight );
		...
		glCopyTexSubImage2D( GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 0, 0, backEnd.viewParms.viewportWidth, backEnd.viewParms.viewportHeight );
		glDisable( GL_TEXTURE_RECTANGLE_ARB );
		glEnable( GL_TEXTURE_2D );
...
const void	*RB_DrawBuffer( const void *data ) {
	...
	if (tr.world && tr.world->globalFog != -1)
	{
		...
		{
		default:
			glClearColor( 1, 0, 0.5, 1 ); // default q3 pink
			break;
		case 1:
...
static void RB_GammaCorrectRender()
{
	...
	GL_SelectTexture(0);
	glBindTexture(GL_TEXTURE_RECTANGLE_ARB, tr.sceneImage);
	glCopyTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA, 0, 0, glConfig.vidWidth, glConfig.vidHeight, 0);
	...
	glBegin(GL_QUADS);
		...
		glTexCoord2f(0.0f, (float)glConfig.vidHeight);
		glVertex2f(-1.0f,  1.0f);

		glTexCoord2f((float)glConfig.vidWidth, (float)glConfig.vidHeight);
		glVertex2f( 1.0f,  1.0f);

		glTexCoord2f((float)glConfig.vidWidth, 0.0f);
		glVertex2f( 1.0f, -1.0f);
	glEnd();
...
static inline void RB_BlurGlowTexture()
{
	...
	glActiveTextureARB( GL_TEXTURE3_ARB );
	glEnable( GL_TEXTURE_RECTANGLE_ARB );
	glBindTexture( GL_TEXTURE_RECTANGLE_ARB, uiTex );
...
/*	else
	{
		...
		glBegin(GL_QUADS);
			glColor4f( 1.0f, 1.0f, 1.0f, 1.0f );
			glMultiTexCoord2fARB( GL_TEXTURE1_ARB, 0, iTexHeight );
			glMultiTexCoord2fARB( GL_TEXTURE0_ARB, 0, glConfig.vidHeight );
			glVertex2f( 0, 0 );
			...
			glMultiTexCoord2fARB( GL_TEXTURE1_ARB, iTexWidth, 0 );
			glMultiTexCoord2fARB( GL_TEXTURE0_ARB, glConfig.vidWidth, 0 );
			glVertex2f( glConfig.vidWidth, glConfig.vidHeight );

			glMultiTexCoord2fARB( GL_TEXTURE1_ARB, iTexWidth, iTexHeight );
			glMultiTexCoord2fARB( GL_TEXTURE0_ARB, glConfig.vidWidth, glConfig.vidHeight );
			glVertex2f( glConfig.vidWidth, 0 );
		glEnd();
